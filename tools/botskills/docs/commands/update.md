...
```bash
botskills update --localManifest "./skills/customSkill/customSkillManifest.json" --appSettingsFile "./appsettings.json" --cs
```
...
```bash
botskills update --remoteManifest "http://<YOUR_SKILL_MANIFEST>.azurewebsites.net/api/skill/manifest" --appSettingsFile "./appsettings.json" --cs
```