    ...
    public class MainDialog : ActivityHandlerDialog
    {
        ...
        protected override async Task OnMessageActivityAsync(DialogContext innerDc, CancellationToken cancellationToken = default)
        {
            ...
            if (!string.IsNullOrEmpty(activity.Text))
            {
                ...
                localizedServices.LuisServices.TryGetValue("FoodOrderSkill", out var luisService);

                if (luisService != null)
                {
                    var result = innerDc.Context.TurnState.Get<FoodOrderSkillLuis>(StateProperties.SkillLuisResult);
                    var intent = result?.TopIntent().intent;

                    switch (intent)
                    {
                        case FoodOrderSkillLuis.Intent.RepeatOrder:
                            {
