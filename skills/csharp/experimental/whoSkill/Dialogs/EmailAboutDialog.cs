...
using WhoSkill.Responses.EmailAbout;
...
namespace WhoSkill.Dialogs
{
    public class EmailAboutDialog : WhoSkillDialogBase
    {
        ...
        protected override async Task<DialogTurnResult> SearchKeyword(WaterfallStepContext sc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            // This dialog don't need search candidates first, because it searchs current user's email.
        ...
        protected override async Task<DialogTurnResult> DisplayResult(WaterfallStepContext sc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (state.Results == null)
            {
                ...
                {
                    results = await MSGraphService.GetEmailContacts(state.Keyword);
                }

                if (results == null)
                {
                    var activity = TemplateEngine.GenerateActivityForLocale(EmailAboutResponses.NoPeopleEmailsAboutKeywordRencently, new { Keyword = state.Keyword });
            ...
            if (string.IsNullOrEmpty(state.Keyword))
            {
                var reply = TemplateEngine.GenerateActivityForLocale(
                    EmailAboutResponses.PeopleEmailsRencently,
                    new
                    {
            ...
            {
                var reply = TemplateEngine.GenerateActivityForLocale(
                    EmailAboutResponses.PeopleEmailsAboutKeywordRencently,
                    new
                    {
