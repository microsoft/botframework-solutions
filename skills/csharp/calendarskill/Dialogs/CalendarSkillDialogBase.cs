...
namespace CalendarSkill.Dialogs
{
    public class CalendarSkillDialogBase : ComponentDialog
    {
        ...
        protected async Task<string> GetMyPhotoUrlAsync(ITurnContext context)
        {
            ...
            var service = ServiceManager.InitUserService((string)token, state.EventSource, context);
        ...
        protected async Task<List<PersonModel>> GetContactsAsync(WaterfallStepContext sc, string name)
        {
            ...
            var state = await Accessor.GetAsync(sc.Context);
            sc.Context.TurnState.TryGetValue(APITokenKey, out var token);
            var service = ServiceManager.InitUserService((string)token, state.EventSource, sc.Context);
