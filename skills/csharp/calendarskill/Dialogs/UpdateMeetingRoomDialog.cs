...
namespace CalendarSkill.Dialogs
{
    public class UpdateMeetingRoomDialog : CalendarSkillDialogBase
    {
        public UpdateMeetingRoomDialog(
            ...
            : base(nameof(UpdateMeetingRoomDialog), settings, services, responseManager, conversationState, serviceManager, telemetryClient, appCredentials)
        {
        ...
        private async Task<DialogTurnResult> Prepreocess(WaterfallStepContext sc, CancellationToken cancellationToken = default(CancellationToken))
        {
            try
            {
                var state = await Accessor.GetAsync(sc.Context);

                if (!state.MeetingInfor.StartDate.Any() && state.MeetingInfor.EndDate.Any())
                {
                    state.MeetingInfor.StartDate.AddRange(state.MeetingInfor.EndDate);
                }

                if (!state.MeetingInfor.StartTime.Any() && state.MeetingInfor.EndTime.Any())
                {
                    state.MeetingInfor.StartTime.AddRange(state.MeetingInfor.EndTime);
