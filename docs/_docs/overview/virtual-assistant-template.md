...
You can find the ARM template (template.json) in your **Deployment\Resources** folder or [here]({{site.repo}}/tree/main/templates/csharp/VA/VA/Deployment/Resources). The PowerShell scripts can be found in your **Deployment\Scripts** folder or [here]({{site.repo}}/tree/main/templates/csharp/VA/VA/Deployment/Scripts).
...
1. The **MainDialog** dialog provided in the template derives from a base class called [RouterDialog]({{site.repo}}/blob/main/sdk/csharp/libraries/microsoft.bot.solutions/Dialogs/RouterDialog.cs) which can be found in the  **Microsoft.Bot.Builder.Solutions** NuGet library.
...
A number of middleware components have been provided to address some key scenarios and are included in the **Microsoft.Bot.Builder.Solutions** nuget library or in [this location]({{site.repo}}/blob/main/sdk/csharp/libraries/microsoft.bot.solutions/Middleware).
...
You can find this component within the **Microsoft.Bot.Builder.Solutions** NuGet library or in [this location]({{site.repo}}/blob/main/sdk/csharp/libraries/microsoft.bot.solutions/Middleware/SetLocaleMiddleware.cs).
...
The [**Set Speak Middleware**]({{site.repo}}/blob/main/sdk/csharp/libraries/microsoft.bot.solutions/Middleware/SetSpeakMiddleware.cs) provides these capabilities and only executes when the Direct-Line Speech channel is used.  An example SSML fragment is shown below:
...
It can be hard to generate these activities for testing purposes as the Bot Framework Emulator doesn't provide the ability to send Activities. The [**Event Debug Middleware**]({{site.repo}}/blob/main/sdk/csharp/libraries/microsoft.bot.solutions/Middleware/EventDebuggerMiddleware.cs) provides an elegant workaround enabling you to send messages following a specific format which are then transposed into an Event activity processed by your Assistant
...
[**Content Moderator Middleware**]({{site.repo}}/blob/main/sdk/csharp/libraries/microsoft.bot.solutions/Middleware/ContentModeratorMiddleware.cs)  is provided that screen texts and surfaces output through a **TextModeratorResult** on the **TurnState** object. This middleware is not enabled by default.
...
The **MainDialog** class provided in the template derives from a base class called [RouterDialog]({{site.repo}}/blob/main/sdk/csharp/libraries/microsoft.bot.solutions/Dialogs/RouterDialog.cs) which can be found in the  **Microsoft.Bot.Builder.Solutions** NuGet library.
