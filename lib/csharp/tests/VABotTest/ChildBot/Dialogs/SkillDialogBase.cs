    ...
    public class SkillDialogBase : ComponentDialog
    {
        ...
        protected async Task GetLuisResult(DialogContext dc)
        {
            if (dc.Context.Activity.Type == ActivityTypes.Message)
            {
                ...
                var luisService = localeConfig.LuisServices["SkillBot"];

                // Get intent and entities for activity
                var result = await luisService.RecognizeAsync<SkillBotLuis>(dc.Context, CancellationToken.None);
                state.BotLuisResult = result;
            }
        }
