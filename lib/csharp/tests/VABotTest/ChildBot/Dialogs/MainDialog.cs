...
using ChildBot.Models;
using ChildBot.Responses.Main;
using ChildBot.Responses.Shared;
using ChildBot.Services;
using Luis;
...
namespace ChildBot.Dialogs
{
    public class MainDialog : RouterDialog
    {
        ...
        protected override async Task OnStartAsync(DialogContext dc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            await dc.Context.SendActivityAsync(_responseManager.GetResponse(MainResponses.WelcomeMessage), cancellationToken);
        }

        protected override async Task RouteAsync(DialogContext dc, CancellationToken cancellationToken = default)
        {
            ...
            localeConfig.LuisServices.TryGetValue("SkillBot", out var luisService);
            ...
            else
            {
                ...
                var result = await luisService.RecognizeAsync<SkillBotLuis>(dc.Context, cancellationToken);
                var intent = result?.TopIntent().intent;

                switch (intent)
                {
                    case SkillBotLuis.Intent.Sample:
                        {
                            turnResult = await dc.BeginDialogAsync(nameof(SampleDialog));
                            break;
                        }

                    case SkillBotLuis.Intent.None:
                        {
        ...
        protected override async Task<InterruptionAction> OnInterruptDialogAsync(DialogContext dc, CancellationToken cancellationToken = default)
        {
        ...
        private Task PopulateStateFromSemanticAction(ITurnContext context)
        {
