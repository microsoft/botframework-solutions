...
namespace Microsoft.Bot.Builder.Skills
{
    ...
    public static class SkillRouter
    {
        ...
        {
            var manifest = skillConfiguration.SingleOrDefault(s => s.Actions.Any(a => a.Id == dispatchIntent));

            if (manifest == null)
            {
                manifest = skillConfiguration.SingleOrDefault(s => s.Id == dispatchIntent);
            }
