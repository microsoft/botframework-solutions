...
describe("The disconnect command", function () {
    ...
    beforeEach(function () {
        ...
        fs.writeFileSync(path.resolve(__dirname, path.join('mocks', 'resources', 'filledDispatch.dispatch')),
            JSON.stringify(
                {
                    "services": [
                        {
                ...
                , null, 4));
        fs.writeFileSync(path.resolve(__dirname, path.join('mocks', 'resources', 'filledDispatchNoJson.dispatch')),
            JSON.stringify(
                {
                    "services": [
                        {
                ...
                , null, 4));
        fs.writeFileSync(path.resolve(__dirname, path.join('mocks', 'resources', 'filledSkillsArray.json')),
            JSON.stringify(
                {
                    "skills": [
                        {
	...
	describe("should show an error", function () {
        it("when there is no skills File", async function () {
            const config = {
                ...
                luisFolder : "",
                dispatchFolder : "",
                lgOutFolder: path.resolve(__dirname, '..', 'mocks', 'resources'),
                dispatchName : "",
                lgLanguage : "cs",
                logger : this.logger
            };
        ...
        it("when the skillsFile points to a bad formatted Assistant Skills configuration file", async function () {
            const config = {
                skillId : "testSkill",
                skillsFile: path.resolve(__dirname, 'mocks', 'resources', 'badSkillsArray.jso'),
                outFolder : "",
                cognitiveModelsFile : "",
                language : "",
                luisFolder : "",
        ...
        it("when the dispatchName and dispatchFolder point to a nonexistent file", async function () {
            const config = {
                skillId : "testSkill",
                skillsFile: path.resolve(__dirname, 'mocks', 'resources', 'filledSkillsArray.json'),
                outFolder : "",
                cognitiveModelsFile : "",
                language : "",
                luisFolder : "",
                dispatchFolder : path.resolve(__dirname, 'mocks', 'resources'),
                lgOutFolder: path.resolve(__dirname, 'mocks', 'resources'),
...
Error: There was an error while refreshing the Dispatch model.`);
    ...
    describe("should show a warning", function () {
        it("when the skillsFile points to a bad formatted Assistant Skills configuration file", async function () {
            const config = {
                skillId : "testSkill",
                skillsFile: path.resolve(__dirname, 'mocks', 'resources', 'emptySkillsArray.json'),
                outFolder : "",
                cognitiveModelsFile : "",
                language : "",
                luisFolder : "",
