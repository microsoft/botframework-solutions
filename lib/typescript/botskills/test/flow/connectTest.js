...
describe("The connect command", function () {
	...
	describe("should show an error", function () {
        ...
        it("when the localManifest points to a nonexisting Skill's manifest file", async function () {
            const config = {
                botName: '',
        ...
        it("when the Skill's missing all mandatory fields", async function () {
            const config = {
                botName: '',
...
`Missing property 'actions' of the manifest`
        ...
        it("when the remoteManifest points to a nonexisting Skill's manifest URL", async function() {
            const config = {
                botName: '',
                localManifest: '',
        ...
        it("when the luisFolder leads to a nonexistent folder", async function () {
            ...
            await connector.connectSkill(config);
            const ErrorList = logger.getError();
			assert.strictEqual(ErrorList[ErrorList.length - 1], `There was an error while connecting the Skill to the Assistant:\nError: An error ocurred while updating the Dispatch model:\nError: Path to the LUIS folder (C:\\GitHub\\AI\\lib\\typescript\\botskills\\test\\mockedFiles\\nonexistentLuisFolder) leads to a nonexistent folder.`);
        });
    ...
    describe("should show a warning", function () {
        it("when the Skill's already connected", async function () {
            const config = {
                botName: '',
    ...
    xdescribe("should show a message", function () {
        it.skip("when there's a skill in the Assistant Skills configuration file", async function () {
            const config = {
