...
describe("manifest", function() {
    ...
    describe("desearialize valid manifest file", function() {
        it("verify valid manifest structure", async function(){
            ...
            assert.ok(require(join(__dirname, "mocks", "resources", "manifestTemplate.json")));
        });        
    });
    ...
    describe("desearialize invalid manifest file", function() {
        it("verify invalid manifest structure", async function(){
            ...
            assert.throws(() => { require(join(__dirname, "mocks", "testData", "malformedManifestTemplate.json")) }, SyntaxError);
        });        
    });

    describe("is skill helper", function() {
        it("verify existence for events in the manifest", async function(){
            const skillManifest = require(join(__dirname, "mocks", "resources", "manifestTemplate.json"));
            ...
            assert.strictEqual(undefined, SkillRouter.isSkill(skillManifests, "calendarSkill/MISSINGEVENT"));
        });
    });
})