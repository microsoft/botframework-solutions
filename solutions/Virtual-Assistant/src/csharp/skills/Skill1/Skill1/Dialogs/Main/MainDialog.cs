...
using Skill1.Dialogs.Sample;
using Skill1.Dialogs.Shared;
using Skill1.Dialogs.Shared.DialogOptions;
using Skill1.Dialogs.Shared.Resources;
using Skill1.ServiceClients;

namespace Skill1.Dialogs.Main
{
    public class MainDialog : RouterDialog
    {
        ...
        protected override async Task RouteAsync(DialogContext dc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            localeConfig.LuisServices.TryGetValue("Skill1", out var luisService);
            ...
            else
            {
                ...
                var result = await luisService.RecognizeAsync<Skill1LU>(dc.Context, CancellationToken.None);
                var intent = result?.TopIntent().intent;

                switch (intent)
                {
                    case Skill1LU.Intent.Sample:
                        {
                            await dc.BeginDialogAsync(nameof(SampleDialog), skillOptions);
                            break;
                        }

                    case Skill1LU.Intent.None:
                        {
