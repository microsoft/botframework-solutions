...
using DemoSkill.Dialogs.Sample;
using DemoSkill.Dialogs.Shared;
using DemoSkill.Dialogs.Shared.DialogOptions;
using DemoSkill.Dialogs.Shared.Resources;
using DemoSkill.ServiceClients;

namespace DemoSkill.Dialogs.Main
{
    public class MainDialog : RouterDialog
    {
        ...
        protected override async Task RouteAsync(DialogContext dc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            localeConfig.LuisServices.TryGetValue("DemoSkill", out var luisService);
            ...
            else
            {
                ...
                var result = await luisService.RecognizeAsync<DemoSkillLU>(dc.Context, CancellationToken.None);
                var intent = result?.TopIntent().intent;

                switch (intent)
                {
                    case DemoSkillLU.Intent.Sample:
                        {
                            await dc.BeginDialogAsync(nameof(SampleDialog), skillOptions);
                            break;
                        }

                    case DemoSkillLU.Intent.None:
                        {
