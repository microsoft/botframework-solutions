    ...
    public class SendEmailDialog : EmailSkillDialog
    {
        ...
        public async Task<DialogTurnResult> CollectSubject(WaterfallStepContext sc, CancellationToken cancellationToken = default(CancellationToken))
        {
            try
            {
                ...
                var recipientConfirmedMessage = sc.Context.Activity.CreateReply(SharedResponses.RecipientConfirmed, null, new StringDictionary() { { "UserName", await GetNameListString(sc) } });
        ...
        public async Task<DialogTurnResult> SendEmail(WaterfallStepContext sc, CancellationToken cancellationToken = default(CancellationToken))
        {
            try
            {
                ...
                if (confirmResult)
                {
                    ...
                    var replyMessage = sc.Context.Activity.CreateAdaptiveCardReply(SharedResponses.SentSuccessfully, "Dialogs/Shared/Resources/Cards/EmailWithOutButtonCard.json", emailCard);
